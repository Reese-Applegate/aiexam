...
#          faissFaceSearch ( faiss 정보를 검색한다. )
########################################################################################################
...
def faissFaceLoad(loadFile):
    ...
    try:
        ...
        print("FAISS 데이터베이스를 불러오는데 실패했습니다.")
        exit()
...
def faissFaceSearch(img):
    ...
    test_face = face_recognition.face_encodings(test_img)
    if len(test_face) != 1:
        print("얼굴을 찾을 수 없습니다.")
        exit()
    ...
    # 예측
    distance, result = FAISS_INDEX.search(test_en, k=7)
...
faissFaceSearch('./source/test.png')