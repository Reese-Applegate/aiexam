...
def gen_filename(search):
    ...
    return f"./data/{date_time}_{time_part}_{search}.csv", date_time, time_part


def search_naver_kin(search):  # 총괄 함수 지정.
    print("검색어: ", search)
    ...
    f = open(
        save_file, "w", encoding="cp949", newline=""
    )  # 엑셀파일은 encding=cp949 for preventing broken korean.
    csvWriter = csv.writer(f)  # csv writer 객체, csv방식으로 저장하라.
    csvWriter.writerow(["title", "link"])  # csv파일에 제목과 링크를 저장한다.

    # Paging
    for i in range(1):  # 100은 천개. 하나랑 10개의 데이터가 있었어.
        ...
        # BeautifulSoup 으로 html 파d씽
        soup = bs(html, "html.parser")
        total = soup.select(
            ".basic1 > li > dl > dt > a"
        )  # html 페이지 소스에서 원하는 부분을 parsing.
